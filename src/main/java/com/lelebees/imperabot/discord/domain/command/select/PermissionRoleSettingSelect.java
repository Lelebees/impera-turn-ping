...
public class PermissionRoleSettingSelect implements SelectMenuInteraction {
    ...
    public Mono<Void> handle(SelectMenuInteractionEvent event) {
        ...
        if (guildOptional.isEmpty()) {
            logger.error("User {} ({}) attempted to update permission role for guild but interaction was issued outside one. How did we get here?", callingUser.getUsername(), callingUser.getId().asLong());
        ...
        try {
            ...
            if (!oldSettings.hasPermissionRoleManagePermissions(callingUser)) {
                logger.info("User {} ({}) attempted to update the permission role for guild {} ({}) but was denied because they have no permission", callingUser.getUsername(), callingUser.getId().asLong(), guild.getName(), guild.getId().asLong());
            ...
            logger.info("User {} ({}) updated the permission role for guild {} ({}).", callingUser.getUsername(), callingUser.getId().asLong(), guild.getName(), guild.getId().asLong());
            ...
            logger.warn("User {} ({}) attempted to update permission role for guild {} ({}) but it's settings were not in the database.", callingUser.getUsername(), callingUser.getId().asLong(), guild.getName(), guild.getId().asLong());
